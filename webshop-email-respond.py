...
def authenticate_gmail():
    """Authenticate Gmail API with token refresh support."""
    creds = None
...
def send_response(**kwargs):
    """Send auto-response to unread emails."""
    email_data = kwargs['dag_run'].conf  # Get email details from trigger
    if not email_data:
        raise ValueError("No email data received.")
    ...
    print(f"Auto-response sent to: {recipient}")
